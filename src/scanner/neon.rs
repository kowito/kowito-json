...
pub unsafe fn scan_neon(bytes: &[u8], tape: &mut [u32]) -> usize {
    ...
    while i + 32 <= bytes.len() {
        ...
        let _m_esc = chunk.simd_eq(u8x32::splat(b'\\')).to_bitmask();
        ...
        let quote_mask_u128: u128 = unsafe { vmull_p64(m_quote, !0) };
        let quote_mask = (quote_mask_u128 as u64) ^ prev_in_string;
        let string_mask = quote_mask;
    ...
    if i < bytes.len() {
        ...
        for item in temp_tape.iter().take(tail_count) {
            if tape_idx < tape.len() {
                tape[tape_idx] = (i as u32) + item;
                tape_idx += 1;
            }
        }
    }
